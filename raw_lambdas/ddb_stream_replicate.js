 ...
 *  DDB: blue        arn:aws:dynamodb:us-west-2:210838369981:table/blue/stream/2016-06-17T09:03:32.462    Enabled      Batch size: 100, Last result: OK
 */
...
exports.handler = (event, context, callback) => {
    ...
    let replicaTable = ReplicaTable.create('green'); // <--- HERE GOES YOUR SLAVE TABLE NAME
    
    replicaTable.replicate(records).then(() => {
...
class Record {
    constructor(data) {
        ...
        if (this._data.eventSource !== 'aws:dynamodb') {
            throw new Error(`Trying to initialize DynamoDB stream Record from an invalid event source ${this._data.eventSource}`);
        }
    }
        ...
        return records
            ...
            .map(record => {
                ...
                for (let attributeName in recordData) {
                    if (!recordData.hasOwnProperty(attributeName) || attributeName === 'Id') {
                        continue;
                    }
